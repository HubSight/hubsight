...
export const PasskeySettingsSection: React.FC = () => {
  ...
  return (
    <div className="p-4 bg-slate-50/80 border border-slate-200/80 rounded-2xl space-y-3.5">
      ...
      {/* Notifications */}
      {success && (
        <div className="p-2.5 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 text-xs flex items-center gap-2 animate-fade-in">
      ...
      {error && (
        <div className="p-2.5 bg-red-50 border border-red-200 rounded-xl text-red-600 text-xs flex items-center gap-2 animate-fade-in">
      ...
      <div className="space-y-2 pt-1">
        {loading && passkeys.length === 0 ? (
          ...
          <div className="p-4 border border-dashed border-slate-200 rounded-xl text-center">
            <p className="text-xs text-slate-400">{t('settings.noPasskeys')}</p>
          </div>
        ) : (
          passkeys.map((key) => (
            <div
              key={key.id}
              className="p-3 bg-white border border-slate-200/90 rounded-xl flex items-center justify-between gap-3 shadow-2xs hover:border-slate-300 transition-colors"
            >
              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                <div className="w-8 h-8 rounded-lg bg-orange-50 border border-orange-200/50 flex items-center justify-center text-orange-600 shrink-0">
                  <KeyRound size={16} />
                </div>
                  ...
                  {editingId === key.id ? (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="text-xs font-semibold py-1 px-2 border border-slate-300 rounded-lg w-full focus:outline-none focus:border-orange-500"
                      />
                      <button
                        type="button"
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          ...
          <div className="relative w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-2xl p-5 z-10 space-y-4">
            ...
            <form onSubmit={handleRegisterPasskey} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:bg-white focus:outline-none focus:border-orange-500"
                />
              </div>
