...
export const DeviceRtspTab: React.FC<DeviceRtspTabProps> = ({ formData, onChange, isStreaming = false }) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1.5">{t('device.defaultRtspPort')}</label>
        <input
          type="number"
          value={formData.rtspPort}
          onChange={(e) => handlePortChange(e.target.value)}
          disabled={isStreaming}
          className={`input-field w-full ${isStreaming ? 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400' : ''}`}
          placeholder="554"
        />
