...
export const ArchiveSidebar: React.FC<ArchiveSidebarProps> = ({
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 lg:gap-6 bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">

      {/* Camera Selection */}
      <div className="flex items-center gap-3">
        <label className="flex items-center gap-1.5 text-sm text-slate-600 dark:text-slate-300 font-medium whitespace-nowrap">
      ...
      <div className="flex items-center gap-3 relative" ref={calendarRef}>
        ...
        <button
          className={`input-field bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 min-w-[150px] py-1.5 flex justify-between items-center ${!selectedCam ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer'
            }`}
          onClick={() => selectedCam && setShowCalendar(!showCalendar)}
          disabled={!selectedCam}
        >
          <span className="text-slate-700 dark:text-slate-200">{dayjs(dateObj).locale(locale).format(locale === 'vi' ? 'DD/MM/YYYY' : 'DD MMM, YYYY')}</span>
      ...
      <div className="w-px h-8 bg-slate-200 dark:bg-slate-800 hidden md:block mx-2"></div>

      {/* Recording Status */}
      <div className="flex items-center gap-2 text-sm ml-auto">
        <span className="text-slate-500 dark:text-slate-400 font-medium">{t('playback.recordings')}</span>
        {loading ? (
          <span className="text-slate-400">{t('loading')}</span>
        ) : recordings.length === 0 ? (
          <span className="text-slate-400">{t('noData')}</span>
        ) : (
          <span className="text-orange-600 dark:text-orange-400 font-bold bg-orange-50 dark:bg-orange-950/40 px-2 py-0.5 rounded-md">
      ...
      {/* Multi-View Link */}
      <div className="w-px h-8 bg-slate-200 dark:bg-slate-800 hidden md:block mx-1"></div>
      <button
        type="button"
        onClick={() => navigate('/multiview')}
        title={t('multiview.switchToMultiView')}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-orange-200 dark:border-orange-800/60 bg-orange-50/70 dark:bg-orange-950/40 text-orange-700 dark:text-orange-300 hover:bg-orange-100 dark:hover:bg-orange-900/40 text-xs font-semibold transition-colors cursor-pointer"
      >
        <LayoutGrid size={15} />
