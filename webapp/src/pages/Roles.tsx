...
export const Roles: React.FC = () => {
  ...
  const handleDeleteRole = async () => {
    if (!pendingDeleteRoleId || isDeletingRole) return;
  ...
  const handleSaveRolePermissions = async () => {
    if (!activeRole || activeRole.code === 'admin' || isSavingPerms) return;
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50/50 dark:bg-slate-950/50 overflow-hidden">
            ...
            <form onSubmit={handleSaveRole} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex items-center justify-end gap-2 border-t border-slate-100 dark:border-slate-800">
                ...
                >
                  ...
                  {isSubmittingRole ? t('common.saving') : t('save')}
                </button>
              </div>
            </form>
          </div>
      ...
      <ConfirmDialog
        ...
        onCancel={() => {
          if (!isDeletingRole) setPendingDeleteRoleId(null);
        }}
      />
    </div>
  );
