    ...
    <div className="w-full flex-1 flex flex-col p-4 sm:p-6 lg:p-8 overflow-y-auto custom-scrollbar">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3.5 my-6">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs flex items-center gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-lg font-bold text-slate-900 dark:text-slate-100">{accounts.length}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs flex items-center gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{t('serviceAccounts.fcmStatus')}</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs flex items-center gap-3.5">
          <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 shrink-0">
            <Clock size={18} />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{t('serviceAccounts.lastTested')}</p>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="hidden md:block bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xs overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[920px] text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/80 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none whitespace-nowrap">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex flex-col gap-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-bold text-slate-800 dark:text-slate-100 text-sm group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors whitespace-nowrap">
                      ...
                      <td className="py-3.5 px-5 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-slate-600 dark:text-slate-300 truncate max-w-[240px]" title={acc.client_email}>
                            {acc.client_email}
                          </span>
                          <button
                        ...
                        <div className="flex items-center justify-end gap-1.5 shrink-0">
                          ...
                          {!acc.is_active && (
                            <button
                              ...
                              className="px-2.5 py-1 text-xs font-semibold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/40 border border-orange-200 dark:border-orange-800 rounded-lg hover:bg-orange-100 dark:hover:bg-orange-900/50 active:scale-95 transition-all cursor-pointer disabled:opacity-50 shrink-0"
                            >
                          ...
                          <button
                            onClick={() => setDetailAccount(acc)}
                            className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-950/40 rounded-lg transition-colors cursor-pointer shrink-0"
          ...
          <div className="block md:hidden space-y-3">
            {filteredAccounts.map((acc) => (
              <div
                key={acc.id}
                onClick={() => setDetailAccount(acc)}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-2xs cursor-pointer hover:border-orange-200 dark:hover:border-orange-800 transition-all space-y-3"
              >
                ...
                <div className="p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/60 dark:border-slate-700/60 font-mono text-xs flex items-center justify-between gap-2">
                  <span className="text-slate-600 dark:text-slate-300 truncate">{acc.client_email}</span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  >
                    {copiedId === `m-email-${acc.id}` ? <Check size={13} className="text-emerald-600 dark:text-emerald-400" /> : <Copy size={13} />}
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
                  <button
                    onClick={() => handleTestConnection(acc)}
                    disabled={testingId === acc.id}
                    className="px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-1.5"
                  >
                    <RotateCw size={12} className={testingId === acc.id ? 'animate-spin text-orange-600 dark:text-orange-400' : ''} />
                  ...
                  <div className="flex items-center gap-2">
                    {!acc.is_active && (
                      <button
                        onClick={() => handleActivate(acc)}
                        disabled={activatingId === acc.id}
                        className="px-3 py-1.5 text-xs font-semibold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/40 border border-orange-200 dark:border-orange-800 rounded-xl hover:bg-orange-100 dark:hover:bg-orange-900/50"
                      >
                        {t('serviceAccounts.activateBtn')}
                      </button>
                    )}
                    <button
                      onClick={() => setDeleteAccount(acc)}
                      className="p-2 text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 rounded-xl"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
            ...
            <form onSubmit={handleImportSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
              ...
              {importTab === 'upload' && (
                <div>
                  ...
                  <label
                    ...
                    className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-orange-500 dark:hover:border-orange-500 hover:bg-orange-50/20 dark:hover:bg-orange-950/20 rounded-2xl p-6 text-center cursor-pointer transition-all"
                  >
                    <div className="w-12 h-12 rounded-2xl bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 flex items-center justify-center mb-2">
                      <Upload size={22} />
                    </div>
                    {importFile ? (
                      <div>
                      ...
                      <div>
                        ...
                        <span className="inline-block mt-1.5 text-xs font-semibold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/40 hover:bg-orange-100 dark:hover:bg-orange-900/50 px-3 py-1 rounded-lg border border-orange-200 dark:border-orange-800 transition-colors">
              ...
              {importTab === 'paste' && (
                <div>
                  <textarea
                    ...
                    className="w-full p-3 font-mono text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all resize-y"
                  />
                </div>
              )}
              ...
              {parsedPreview && !jsonValidationError && (
                <div className="p-3.5 rounded-xl bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60 text-xs text-emerald-900 dark:text-emerald-200 space-y-1.5 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] pt-1">
                    <div>
                      <span className="text-emerald-600 dark:text-emerald-400 font-medium">Project ID:</span>{' '}
                      <span className="font-mono font-bold text-emerald-950 dark:text-emerald-100">{parsedPreview.project_id}</span>
                    </div>
                    <div>
                      <span className="text-emerald-600 dark:text-emerald-400 font-medium">Key ID:</span>{' '}
                      <span className="font-mono text-emerald-950 dark:text-emerald-100">{parsedPreview.private_key_id?.slice(0, 10)}...</span>
                    </div>
                    <div className="col-span-2 truncate">
                      <span className="text-emerald-600 dark:text-emerald-400 font-medium">Email:</span>{' '}
                      <span className="font-mono text-emerald-950 dark:text-emerald-100">{parsedPreview.client_email}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs sm:text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all"
                />
              </div>
      ...
      {detailAccount && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          ...
          <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/80 shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">{t('serviceAccounts.detailSubtitle')}</p>
                </div>
              </div>
              <button
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/70 dark:border-slate-700/60">
                    ...
                    <span className="font-mono font-bold text-slate-800 dark:text-slate-100 text-sm">{detailAccount.project_id}</span>
                  </div>
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/70 dark:border-slate-700/60">
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/70 dark:border-slate-700/60">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[11px] text-slate-400 dark:text-slate-500 font-medium">{t('serviceAccounts.clientEmail')}</span>
                    <button
                ...
                {detailAccount.client_id && (
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/70 dark:border-slate-700/60">
                    <span className="text-[11px] text-slate-400 dark:text-slate-500 block font-medium">{t('serviceAccounts.clientId')}</span>
                    <span className="font-mono text-slate-800 dark:text-slate-100 text-xs">{detailAccount.client_id}</span>
                  </div>
                )}
                ...
                <div className={`p-3.5 rounded-xl border flex items-start gap-3 ${
                  ...
                  {detailAccount.status === 'active' ? (
                    ...
                    <Clock size={18} className="text-slate-400 dark:text-slate-500 shrink-0 mt-0.5" />
                  )}
            ...
            {/* Modal Footer */}
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/60 shrink-0">
              <button
                onClick={() => handleTestConnection(detailAccount)}
                disabled={testingId === detailAccount.id}
                className="px-3.5 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 active:scale-95 transition-all cursor-pointer flex items-center gap-1.5"
              >
                <RotateCw size={13} className={testingId === detailAccount.id ? 'animate-spin text-orange-600 dark:text-orange-400' : ''} />
