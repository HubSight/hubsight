...
const (
	...
	PurposeThumb StreamPurpose = "thumb" // Persistent 640p 15FPS stream for thumbnail / snapshot
...
type StreamConnection struct {
	ID          string        `json:"id"` // e.g. conn_{id}_cv (#0), conn_{id}_nvr (#1), conn_{id}_thumb, conn_{id}_live_2 (#2+)
...
type CameraPool struct {
	...
	ThumbConnection *StreamConnection            `json:"thumb_connection"` // Persistent 640p 15FPS stream for thumbnail
	CVConnection    *StreamConnection            `json:"cv_connection"`    // Connection #0 (Always active for CV if enable_ai)
...
type PoolStatusSummary struct {
	...
	TotalThumbStreams  int           `json:"total_thumb_streams"`
