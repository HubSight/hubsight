...
export class VideoRTC extends HTMLElement {
    constructor() {
         ...
         * Default to pure WebRTC for ultra-low latency playback.
         * @type {string}
         */
        this.mode = 'webrtc';
    ...
    onwebrtc() {
        ...
        pc.addEventListener('connectionstatechange', () => {
            if (pc.connectionState === 'connected') {
                ...
                // Attaching an audio track with no incoming packets stalls Chrome video playback on A/V sync
                const videoTracks = tracks.filter(t => t.kind === 'video');
                const audioTracks = tracks.filter(t => t.kind === 'audio');
                const mediaStream = new MediaStream(videoTracks);
