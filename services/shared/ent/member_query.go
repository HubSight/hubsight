...
func (_q *MemberQuery) QueryFaces() *MemberFaceQuery {
	query := (&MemberFaceClient{config: _q.config}).Query()
...
func (_q *MemberQuery) WithFaces(opts ...func(*MemberFaceQuery)) *MemberQuery {
	query := (&MemberFaceClient{config: _q.config}).Query()
...
func (_q *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	...
	if query := _q.withFaces; query != nil {
		if err := _q.loadFaces(ctx, query, nodes,
			func(n *Member) { n.Edges.Faces = []*MemberFace{} },
			func(n *Member, e *MemberFace) { n.Edges.Faces = append(n.Edges.Faces, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MemberQuery) loadFaces(ctx context.Context, query *MemberFaceQuery, nodes []*Member, init func(*Member), assign func(*Member, *MemberFace)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(memberface.FieldMemberID)
	}
	query.Where(predicate.MemberFace(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(member.FacesColumn), fks...))
	}))
