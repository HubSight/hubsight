...
type MemberEdges struct {
	...
	Faces []*MemberFace `json:"faces,omitempty"`
...
func (e MemberEdges) FacesOrErr() ([]*MemberFace, error) {
	if e.loadedTypes[0] {
		return e.Faces, nil
	}
...
func (*Member) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case member.FieldID, member.FieldName, member.FieldRole, member.FieldAvatarURL:
	...
	for i := range columns {
		...
		case member.FieldRole:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Role = member.Role(value.String)
			}
...
// QueryFaces queries the "faces" edge of the Member entity.
func (_m *Member) QueryFaces() *MemberFaceQuery {
	return NewMemberClient(_m.config).QueryFaces(_m)
}
