	...
	for i := range columns {
		...
		case setting.FieldNvrStatus:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field nvr_status", values[i])
			} else if value.Valid {
				_m.NvrStatus = value.Bool
			}
		case setting.FieldStorageQuotaGB:
...
func (_m *Setting) String() string {
	...
	builder.WriteString("nvr_status=")
	builder.WriteString(fmt.Sprintf("%v", _m.NvrStatus))
	builder.WriteString(", ")
